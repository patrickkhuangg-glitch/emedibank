...
export function reviewLibrary(responses:ReviewResponse[],query:{q?:string;format?:string;status?:string;page?:string}){
 ...
 for(const response of responses){
  ...
  const entry=groups.get(key)??{id:key,title:response.mock?(response.format==='mmi'?'Full MMI · 8 stations':'Full panel interview'):response.title,format:response.format,createdAt:response.createdAt,responses:[],full:!!response.mock}
