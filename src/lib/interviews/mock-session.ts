...
import { makeMockSteps, makeTrialMockSteps } from './mock-plan'
...
export function startMockSession(userId:string, selection:MockSelection, now=Date.now(), trial=false, id:string=randomUUID()) {
 const steps=trial?makeTrialMockSteps(selection):makeMockSteps(selection)
 const ticket:MockTicket={version:1,id,userId,startedAt:now,format:selection.format,mode:selection.mode,steps}
