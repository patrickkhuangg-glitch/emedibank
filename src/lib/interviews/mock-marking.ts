...
/** A panel can be reviewed as an excerpt; every saved member must still be valid. */
export function validPanelMock<T extends {format:'mmi'|'panel';station_snapshot:unknown}>(attempts:T[],id:string) {
 const members=attempts.map(a=>mockMembership(a.station_snapshot,a.format))
 return attempts.length>=2&&attempts.length<=10&&attempts.every(a=>a.format==='panel')&&members.every(m=>m?.id===id)&&new Set(members.map(m=>m?.index)).size===attempts.length
}
export type MockMarkingSummary={assessmentUnit?:'mmi_station'|'panel_response'|'panel_complete';sessionId:string;createdAt:string;format:'mmi'|'panel';total:number;saved:number;remaining:number;cost:number;credits:number;ready:boolean;submitted:boolean}
