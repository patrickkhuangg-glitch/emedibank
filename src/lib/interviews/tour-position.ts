...
export function positionTour(target:TourRect,viewport:{width:number;height:number},panel:{width:number;height:number}){
 ...
 if(viewport.width>=760&&right+gap+width<=viewport.width-margin){
  x=right+gap;y=clamp(target.top+target.height/2-height/2,margin,viewport.height-height-margin);side='left'
 }else if(viewport.width>=760&&left-gap-width>=margin){
  x=left-gap-width;y=clamp(target.top+target.height/2-height/2,margin,viewport.height-height-margin);side='right'
...
function roundedCutout(rect:TourRect){
...
/** Rounded clear windows keep both the current feature and its workspace tab legible. */
export function tourCutout(rect:TourRect,width:number,height:number,additional:TourRect[]=[]){
 return `path(evenodd, "M0 0 H${width} V${height} H0 Z ${[rect,...additional].map(roundedCutout).join(' ')}")`
}
