import { isRoleplayStation, ROLEPLAY_ASSESSMENT_SCOPE } from './roleplay'
import { recordingAvailable, RECORDING_RETENTION_DAYS } from './recording-retention'
...
export function stationSnapshot(format:unknown, stationId:unknown, questionIndex=0, context: 'standard' | 'daily' = 'standard') {
 ...
 const timing=getInterviewTiming(station.format,context)
 return {...(isRoleplayStation(station)?{response_mode:station.responseMode,response_phases:[{question_index:0,duration_seconds:240},{question_index:1,duration_seconds:240}],assessor_brief:ROLEPLAY_ASSESSMENT_SCOPE}:{}),station_id:station.id,format:station.format,title:station.title,category:station.category,preparation:station.preparation,questions:station.format==='panel'?[station.questions[questionIndex]]:getInterviewQuestions(station),question_index:questionIndex,timing:{preparation_seconds:timing.preparationSeconds,response_seconds:timing.responseSeconds},snapshot_version:1}
}
export function validateDuration(value:unknown, format:'mmi'|'panel', responseSeconds=getInterviewTiming(format).responseSeconds):number {
 if(typeof value!=='number'|| !Number.isFinite(value)||value<=0||value>responseSeconds+10) throw new Error('Recording duration is outside the allowed response time.')
...
export function canSubmit(a:Pick<InterviewAttemptRow,'upload_status'|'marking_status'|'video_deleted_at'|'recording_expires_at'>) { return recordingAvailable(a)&&a.marking_status===null }
...
export function retentionDays(value:string|undefined) { void value; return RECORDING_RETENTION_DAYS }
export function retentionEligible(a:Pick<InterviewAttemptRow,'media_kind'|'marking_status'|'released_at'|'created_at'|'video_deleted_at'|'upload_status'|'recording_expires_at'>,now:number,days=7) {
 if(a.video_deleted_at||a.upload_status!=='ready') return false
 if(a.marking_status && !['released','ungradable'].includes(a.marking_status)) return false
 return Date.parse(a.recording_expires_at??new Date(Date.parse(a.created_at)+days*86400000).toISOString())<=now
}
