import { isMMIFeedback,validateMMIFeedback,type MMIFeedback,type MMIAssessment } from './mmi-feedback'
...
export type InterviewFeedbackData=Feedback|MMIFeedback
export type InterviewAssessment=Assessment|MMIAssessment
...
/** Generation uses only the requested format; stored legacy reports remain readable. */
export function legacyMarkingSchemaFor(format:InterviewFormat,stage:'assess'|'audit'):Schema {
 const allowed=Object.keys(format==='panel'?PANEL_DOMAINS:{...PART_A_DOMAINS,...PART_B_DOMAINS})
 function scope(schema:Schema):Schema {
  return {...schema,properties:schema.properties?Object.fromEntries(Object.entries(schema.properties).map(([name,value])=>[name,name==='key'||name==='domain_key'?(name==='domain_key'&&value.anyOf?nullable({type:'string',enum:allowed}):{type:'string',enum:allowed}):name==='format'?{type:'string',enum:[format]}:scope(value)])):undefined,items:schema.items?scope(schema.items):undefined,anyOf:schema.anyOf?.map(scope)}
 }
 return scope(stage==='assess'?ASSESSMENT_SCHEMA:AUDIT_SCHEMA)
}
export function validateSchema(value:unknown,rawSchema:unknown):boolean {
 const s=rawSchema as Schema
...
export function validateFeedback(value:unknown,format?:InterviewFormat):InterviewFeedbackData {
 if(isMMIFeedback(value)){if(format==='panel')throw new Error('invalid_panel_version');return validateMMIFeedback(value)}
...
export function validateAssessment(value:unknown,format:InterviewFormat,stationId:string,questionCount:number,duration:number):InterviewAssessment {
 if(value&&typeof value==='object'&&'feedback' in value&&isMMIFeedback(value.feedback)){
  const a=value as MMIAssessment
  if(format!=='mmi'||a.format!==format||a.station_id!==stationId||Object.keys(a).some(k=>!['format','station_id','feedback'].includes(k)))throw new Error('invalid_mmi_assessment')
  validateMMIFeedback(a.feedback,{stationId});return a
 }
...
export function validateDraftFeedback(value:unknown):InterviewFeedbackData {
 if(isMMIFeedback(value))return validateMMIFeedback(value,{draft:true})
