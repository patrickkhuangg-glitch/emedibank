import { INTERVIEW_STATION_METADATA as INTERVIEW_STATIONS } from './station-metadata'
...
const stations = new Map<string, (typeof INTERVIEW_STATIONS)[number]>(INTERVIEW_STATIONS.map(station => [station.id, station]))
...
export function suggestPractice(logs: PracticeLog[], today: string, allowedStationIds?: readonly string[]): PracticeSuggestion[] {
  const catalogue = allowedStationIds ? INTERVIEW_STATIONS.filter(station => allowedStationIds.includes(station.id)) : INTERVIEW_STATIONS
  const themes = [...new Set(catalogue.map(station => themeForStation(station.id)))].sort()
  ...
  const candidates = themes.map(theme => {
    ...
    const station = catalogue.filter(station => themeForStation(station.id) === theme).sort((a, b) => recent.filter(log => log.station_id === a.id).length - recent.filter(log => log.station_id === b.id).length || a.id.localeCompare(b.id))[0]
