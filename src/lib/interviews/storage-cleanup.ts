...
export async function cleanupTranscriptionAudio(){
 ...
 if(!a.transcription_audio_path)continue
 const {error:removeError}=await db.storage.from(BUCKET).remove([a.transcription_audio_path])
 if(removeError){failed=true;console.warn(JSON.stringify({event:'interview',attemptId:a.id,stage:'audio_cleanup',outcome:'removal_retry_required'}));continue}
 const {error:updateError}=await db.from('interview_attempts').update({transcription_audio_path:null}).eq('id',a.id).eq('transcription_audio_path',a.transcription_audio_path)
 if(updateError){failed=true;console.warn(JSON.stringify({event:'interview',attemptId:a.id,stage:'audio_cleanup',outcome:'pointer_retry_required'}))}
 }
 // Keep pointers on failed removal, and let the cleanup heartbeat flag the run.
 if(failed)throw new Error('transcription_audio_cleanup_failed')
}

// New practice audio uses the same upload shells. Reap abandoned shells only;
// Saved recordings use the media-only retention queue, which keeps transcripts.
export async function cleanupPracticeAudioUploads(now = Date.now()) {
 ...
 const {data:attempts,error}=await db.from('interview_attempts').select('id').eq('media_kind','audio').eq('station_snapshot->>source','practice_audio').in('upload_status',['awaiting_upload','uploading','failed']).lt('created_at',cutoff).limit(100)
 ...
 for(const candidate of attempts??[]){
  ...
  const {data:reserved,error:reserveError}=await db.from('interview_attempts').update({upload_status:'discarded'}).eq('id',candidate.id).eq('media_kind','audio').eq('station_snapshot->>source','practice_audio').in('upload_status',['awaiting_upload','uploading','failed']).lt('created_at',cutoff).select('*').maybeSingle()
