...
export async function POST(request: Request) {
  try {
    ...
    await requireInterviewPractice(user,station.id,Number(body.questionIndex ?? 0),true)
    ...
    if(body.recording===true)return Response.json({started:true},{headers:{'Cache-Control':'private, no-store'}})
