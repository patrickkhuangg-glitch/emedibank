  ...
  try {
    ...
    if (body.action === 'complete') {
      ...
      if (!log.completed_at) {
        ...
        if (dailyMode) {
          const { data: assigned } = await db.from('interview_daily_stations').select('id').eq('user_id', user.id).eq('local_day', practiceDay(new Date())).eq('station_id', log.station_id).maybeSingle()
          if (!assigned) throw new InterviewApiError('Today’s Daily Station has changed. Return to the dashboard and open it again.', 409)
        }
        const timing = getInterviewTiming(log.format, dailyMode ? 'daily' : 'standard')
        const availableSeconds = Math.floor((Date.now() - Date.parse(log.started_at)) / 1000) - timing.preparationSeconds
        if (typeof body.durationSeconds !== 'number' || !Number.isInteger(body.durationSeconds) || body.durationSeconds < 1 || body.durationSeconds > timing.responseSeconds || availableSeconds < 1 || body.durationSeconds > availableSeconds + 2) throw new InterviewApiError('Complete some spoken practice before saving it to your calendar.')
