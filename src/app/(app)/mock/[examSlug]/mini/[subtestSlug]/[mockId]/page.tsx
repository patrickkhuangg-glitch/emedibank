...
import { findMiniMock, ucatMockTitle } from '@/lib/mock/config'
...
export default async function MiniMockRunPage({ params }: { params: Promise<{ examSlug: string; subtestSlug: string; mockId: string }> }) {
  ...
  const token = signManifest({ u: user.id, e: exam.id, k: mock.assignmentKey, q: resolved[0].questionIds })
  return <MockRunner kind="mini" label={ucatMockTitle(mock)} examSlug={exam.slug} token={token} sections={resolved.map((s) => ({ name: s.name, subtestSlug: s.subtestSlug, qrTopScoreRaw: s.qrTopScoreRaw, minutes: s.minutes, questionIds: s.questionIds }))} />
}
