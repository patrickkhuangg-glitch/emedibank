...
import {canSubmit} from '@/lib/interviews/video-validation'
...
import { InterviewProgressionReview } from '@/components/interviews/progression-review'
...
export default async function PracticeRecordingsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const { data: rows, error } = await db.from('interview_attempts').select('id,format,station_title,created_at,duration_seconds,upload_status,video_deleted_at,recording_expires_at,marking_status').eq('user_id', user.id).eq('media_kind', 'audio').order('created_at', { ascending: false })
  const responses: ReviewResponse[] = (rows ?? []).map(row => ({ id: row.id, format: row.format, title: row.station_title, createdAt: row.created_at, duration: row.duration_seconds, mock: null, status: recordingAvailable(row) ? 'saved' : 'unavailable', eligible: false }))
  ...
  const { data: selected } = id && responses.some(row => row.id === id) ? await db.from('interview_attempts').select('id,format,station_id,station_title,created_at,duration_seconds,upload_status,video_deleted_at,recording_expires_at,marking_status,recording_path,question_events,transcription_status,transcript,questions,approved_feedback').eq('id', id).eq('user_id', user.id).eq('media_kind', 'audio').maybeSingle() : { data: null }
  const [{ data: media }, { data: activity }] = selected ? await Promise.all([
    recordingAvailable(selected) ? db.storage.from('interview-recordings').createSignedUrl(selected.recording_path, recordingUrlLifetime(selected)) : Promise.resolve({ data: null }),
    db.from('interview_practice_logs').select('self_rating').eq('id', selected.id).eq('user_id', user.id).maybeSingle(),
  ]) : [{ data: null }, { data: null }]
  const [firstDaily, retryDaily] = selected ? await Promise.all([
    db.from('interview_daily_stations').select('id').eq('user_id', user.id).eq('first_attempt_log_id', selected.id).maybeSingle(),
    db.from('interview_daily_stations').select('id').eq('user_id', user.id).eq('retry_log_id', selected.id).maybeSingle(),
  ]) : [{ data: null }, { data: null }]
  const progressionDaily = firstDaily.data ?? retryDaily.data
  const profile=await getProfile()
  let feedback=null
  if(selected?.marking_status==='released')try{feedback=validateFeedback(selected.approved_feedback,selected.format)}catch{}
  return <main className="page-frame page-shell space-y-6">
    <header><h1 className="page-title">Practice recordings &amp; transcripts</h1><p className="mt-4 text-muted">Listen back, review your wording and keep notes for your next response. Your practice audio is private.</p></header>
    ...
    {error ? <p role="alert">Your recordings could not load. Please refresh and try again.</p> : selected ? <>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        <article className="min-w-0 space-y-5 rounded-2xl bg-surface p-6">
          ...
          <RecordingRetentionNotice expiresAt={selected.recording_expires_at} pending={markingProtectsRecording(selected.marking_status)} expired={recordingExpired(selected)} />
          {recordingExpired(selected)?null:selected.upload_status !== 'ready' ? <p>Upload unfinished. Return to the recording tab and resume saving your audio.</p> : <InterviewMediaPlayer key={selected.id} attemptId={selected.id} kind="audio" url={media?.signedUrl ?? null} events={Array.isArray(selected.question_events) ? selected.question_events as QuestionEvent[] : []} />}
          ...
          {feedback&&<InterviewFeedback feedback={feedback} format={selected.format}/>}
          {progressionDaily && <InterviewProgressionReview attemptId={selected.id} transcriptReady={selected.transcription_status === 'ready' && Boolean(selected.transcript)} stationId={selected.station_id} format={selected.format} />}
          {selected.marking_status&&<p role="status" className="text-sm text-muted">{selected.marking_status==='released'?'Feedback ready':selected.marking_status==='ungradable'?'Unable to mark — credits refunded':'Submitted for tutor marking'}</p>}
          <InterviewStudentActions key={`mark-${selected.id}`} id={selected.id} format={selected.format} eligible={canSubmit(selected)} credits={profile?.mmi_credits??0}/>
