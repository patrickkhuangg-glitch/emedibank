...
export default async function InterviewReviewPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const [profile,{data:attempts,error}]=await Promise.all([getProfile(),supabase.from('interview_attempts').select(libraryColumns).eq('user_id',user.id).order('created_at',{ascending:false}).returns<LibraryRow[]>()])
