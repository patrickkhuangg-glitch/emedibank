...
export default async function BookingsPage() {
  ...
  return <Container className="py-10 sm:py-14"><main className="mx-auto max-w-6xl">
    ...
    {adminView ? <section className="mt-8 rounded-3xl bg-ink p-6 text-white sm:p-8"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-mono text-xs font-semibold uppercase tracking-[0.16em] text-mint">Admin scheduling</p><h2 className="mt-3 font-display text-2xl font-bold tracking-tight">Book a lesson</h2><p className="mt-2 max-w-xl text-sm leading-6 text-white/70">Select a student package, assign a tutor and choose the tutoring hours to use. Zoom is created automatically.</p></div>{!isZoomConfigured() ? <Link href="/admin/zoom" className="eb-press inline-flex w-fit rounded-full bg-white px-4 py-2 text-sm font-semibold text-ink">Connect Zoom first</Link> : null}</div>{isZoomConfigured() ? <BookingForm plans={bookingPlans} tutors={bookingTutors} /> : null}</section> : null}

    <SessionGroup title="Upcoming" empty={staffView ? 'No tutoring lessons are assigned yet.' : 'No lessons are booked yet. Your tutor will add your next session here.'} sessions={upcoming} staffView={staffView} studentNames={studentNames} />
    <SessionGroup title="Past lessons" empty="There are no completed lessons to review yet." sessions={past} staffView={staffView} studentNames={studentNames} />
  </main></Container>
}

function SessionGroup({ title, empty, sessions, staffView, studentNames }: { title: string; empty: string; sessions: TutoringSession[]; staffView: boolean; studentNames: Map<string, string> }) {
  return <section className="mt-10"><div className="flex items-center justify-between gap-4"><h2 className="font-display text-2xl font-bold tracking-tight">{title}</h2><span className="rounded-full bg-surface-muted px-3 py-1 text-xs font-semibold text-muted">{sessions.length}</span></div>{sessions.length === 0 ? <div className="mt-4 rounded-3xl border border-dashed border-border bg-surface px-6 py-10 text-sm text-muted">{empty}</div> : <div className="mt-4 grid gap-4">{sessions.map((session) => <SessionCard key={session.id} session={session} staffView={staffView} studentName={studentNames.get(session.student_id)} />)}</div>}</section>
}

function SessionCard({ session, staffView, studentName }: { session: TutoringSession; staffView: boolean; studentName?: string }) {
