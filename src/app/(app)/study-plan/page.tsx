...
function StudentSession({ session }: { session: StudentTutoringSession }) {
  ...
  const statusLabel = session.status === 'completed' ? 'Completed' : session.status === 'cancelled' ? 'Cancelled' : 'Awaiting tutor review'
  return <div className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">{session.title}</p><p className="mt-1 text-sm text-white/65">{date} <span className="px-1 text-white/25">·</span> {formatMinutes(session.booked_minutes)}</p></div>{canJoin ? <div className="flex flex-wrap items-center gap-2"><a href={session.zoom_join_url} target="_blank" rel="noreferrer" className="eb-press inline-flex w-fit rounded-full bg-white px-4 py-2 text-sm font-semibold text-ink transition-transform hover:-translate-y-0.5">Join Zoom</a><Link href="/bookings" className="text-sm font-semibold text-white/75 underline decoration-white/30 underline-offset-4 hover:text-white">Manage booking</Link></div> : <span className="text-sm text-white/60">{statusLabel}</span>}</div>
}
