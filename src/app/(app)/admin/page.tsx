...
export default async function AdminPage() {
  ...
  const [queue, exams, trials, published, drafts, studyPlans, accounts] = await Promise.all([
    getPendingMarkings(),
    supabase.from('exams').select('id', { count: 'exact', head: true }).eq('active', true),
    createAdminClient().from('profiles').select('id', { count: 'exact', head: true }).eq('role', 'student').gt('trial_ends_at', new Date().toISOString()),
  ...
  return <Container className="py-10 sm:py-14"><main className="mx-auto max-w-6xl">
    ...
    <section aria-label="Admin overview" className="mt-7 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-4">
      ...
      <Metric value={trials.count ?? 0} label="Students on free trial" />
    ...
    <section className="mt-10"><div className="flex items-baseline justify-between gap-4"><h2 className="font-display text-2xl font-bold">Workspaces</h2><p className="hidden text-sm text-muted sm:block">Choose the job you need to do</p></div>
      <div className="mt-5 grid gap-4 lg:grid-cols-12">
        <Workspace href="/admin/essays" className="lg:col-span-12" eyebrow="Review queue" title="Mark essays" description="Generate an AI draft, edit it in your own voice, then approve the final feedback for the student." count={queue.length} countLabel="awaiting review" icon={<EssayIcon />} tone="brand" action="Open marking queue" />
        ...
        <Workspace href="/bookings" className="lg:col-span-12" eyebrow="Lesson diary" title="Book and follow up tutoring" description="Schedule lessons from a student package, share the Zoom link, then leave notes and homework for the student to revisit." count={studyPlans.count ?? 0} countLabel="student packages" icon={<CalendarIcon />} tone="brand" action="Open bookings" />
      </div>
    </section>
...
function Workspace({ href, className, eyebrow, title, description, count, countLabel, icon, tone = 'surface', action }: { href: string; className: string; eyebrow?: string; title: string; description: string; count: number; countLabel: string; icon: React.ReactNode; tone?: 'surface' | 'brand' | 'ink'; action: string }) {
  ...
  return <Link href={href} className={`eb-press group relative min-h-64 overflow-hidden rounded-[1.4rem] p-6 transition-transform duration-300 hover:-translate-y-0.5 ${palette} ${className}`}><div className="relative z-10 flex h-full flex-col"><div className="flex items-start justify-between gap-5"><span className={`grid h-12 w-12 place-items-center rounded-2xl ${tone === 'surface' ? 'bg-brand-muted text-brand' : 'bg-white/12 text-white'}`}>{icon}</span><div className="text-right"><p className="font-mono text-2xl font-semibold tabular-nums">{count.toLocaleString()}</p><p className={`text-xs ${secondary}`}>{countLabel}</p></div></div>{eyebrow ? <p className={`mt-8 font-mono text-[11px] font-semibold uppercase tracking-[0.16em] ${secondary}`}>{eyebrow}</p> : null}<h3 className={`${eyebrow ? 'mt-2' : 'mt-8'} font-display text-2xl font-bold tracking-tight`}>{title}</h3><p className={`mt-2 max-w-xl text-sm leading-6 ${secondary}`}>{description}</p><p className="mt-auto inline-flex items-center gap-1.5 pt-6 text-sm font-semibold">{action} <ArrowIcon /></p></div>{tone !== 'surface' && <span className="pointer-events-none absolute -bottom-24 -right-16 h-56 w-56 rounded-full border border-white/10" />}</Link>
}
...
function CalendarIcon() { return <svg width="23" height="23" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden><rect x="4" y="5" width="16" height="15" rx="2"/><path d="M8 3v4M16 3v4M4 10h16M8 14h2M14 14h2"/></svg> }
function ArrowIcon() { return <svg className="transition-transform duration-300 group-hover:translate-x-1" width="15" height="15" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M4 10h12M12 6l4 4-4 4" /></svg> }
