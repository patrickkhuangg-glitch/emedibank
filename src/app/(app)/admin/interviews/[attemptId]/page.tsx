import { isMMIFeedback,validateMMIFeedback,validateMMIAudit,type MMIFeedback,type MMIAudit } from '@/lib/interviews/mmi-feedback'
import { mmiAttemptSource } from '@/lib/interviews/mmi-evidence'
...
export default async function Page({params}:{params:Promise<{attemptId:string}>}){
 ...
 let aiAssessment:MMIFeedback|null=null,audit:MMIAudit|null=null
 try{const proposed=m.ai_assessment as {feedback?:unknown}|null;if(isMMIFeedback(proposed?.feedback))aiAssessment=validateMMIFeedback(proposed.feedback)}catch{}
 try{if(aiAssessment)audit=validateMMIAudit(m.evidence_audit,aiAssessment)}catch{}
 const snapshot=a.station_snapshot as {preparation?:string;questions?:string[];title?:string}
 return <main className="page-frame page-shell space-y-6"><Link href="/admin/interviews">← Interview queue</Link><h1 className="page-title">{a.station_title}</h1><p>{detail.student} · {a.format.toUpperCase()} · {a.duration_seconds} seconds · Submitted {a.submitted_for_marking_at?new Date(a.submitted_for_marking_at).toLocaleString('en-AU'):'—'}</p>
 ...
 {a.format!=='mmi'&&<section className="rounded-2xl bg-surface p-5"><h2 className="font-display text-xl font-semibold">Station at the time of recording</h2><p className="mt-3 whitespace-pre-wrap leading-7">{snapshot?.preparation??'Historical audio attempt; preparation text was not saved.'}</p><ol className="mt-3 list-decimal space-y-2 pl-5">{(snapshot?.questions??(Array.isArray(a.questions)?a.questions:[])).map((q,i)=><li key={i}>{String(q)}</li>)}</ol></section>}
 {a.format!=='mmi'&&<InterviewMediaPlayer attemptId={a.id} url={detail.mediaUrl} kind={a.media_kind} events={Array.isArray(a.question_events)?a.question_events as QuestionEvent[]:[]}/>}
 {a.format!=='mmi'&&(a.transcript&&a.transcription_status==='ready'?<GroupedInterviewTranscript key={a.id} attemptId={a.id} transcript={a.transcript} questions={a.questions} reviewer/>:<section><h2 className="font-display text-xl font-semibold">Transcript</h2><p className="mt-3 leading-7">No transcript available. You can review the recording and mark manually.</p></section>)}
 {(a.format!=='mmi'||!aiAssessment)&&<><details className="rounded-2xl border p-4"><summary>Raw primary assessment (private)</summary><pre className="mt-3 overflow-x-auto whitespace-pre-wrap break-words text-sm">{JSON.stringify(m.ai_assessment,null,2)??'No AI assessment'}</pre></details><details open className="rounded-2xl border p-4"><summary>Evidence-audit warnings (private)</summary><pre className="mt-3 overflow-x-auto whitespace-pre-wrap break-words text-sm">{JSON.stringify(m.evidence_audit,null,2)??'Audit unavailable; independently verify all evidence.'}</pre></details></>}
 <AdminInterviewReview recording={a.format==='mmi'?<InterviewMediaPlayer attemptId={a.id} url={detail.mediaUrl} kind={a.media_kind} events={Array.isArray(a.question_events)?a.question_events as QuestionEvent[]:[]}/>:undefined} aiAssessment={aiAssessment} audit={audit} mmiSource={a.format==='mmi'?mmiAttemptSource(a):undefined} stationId={a.station_id} mockEntries={detail.mock&&a.format==='mmi'?detail.responses.map(r=>({id:r.id,index:r.index,title:r.station_title,status:r.marking_status??'pending',feedback:r.approved_feedback})):undefined} key={`${m.lock_version}:${m.updated_at}`} id={a.id} format={a.format} initialFeedback={feedback} initialVersion={m.lock_version} initialNotes={m.private_reviewer_notes??''} initialCorrections={m.transcript_correction_notes??''} terminal={['released','ungradable'].includes(m.status)} transcriptionStatus={a.transcription_status} hasTranscript={!!a.transcript?.trim()} responseDisposition={a.response_disposition} markingStatus={a.marking_status}/>
