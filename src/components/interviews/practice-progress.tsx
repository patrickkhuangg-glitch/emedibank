...
export function InterviewPracticeProgress({ data, notes, showWeeklySummary = true }: { data: PracticeProgressData; notes: ReactNode; showWeeklySummary?: boolean }) {
  ...
  return <>
    <section data-interview-tour="practice-calendar" className="mt-7 rounded-3xl border border-border bg-surface p-5 sm:p-8" aria-labelledby="practice-calendar-title">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><h2 id="practice-calendar-title" className="font-display text-2xl font-semibold tracking-tight">Your practice calendar</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Select a day to explore your activity and that week’s themes.</p></div>
    ...
    <div className={`mt-7 grid items-start gap-7 ${showWeeklySummary ? 'xl:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]' : ''}`}>
      {showWeeklySummary && <section className="min-w-0 rounded-3xl border border-border bg-surface p-5 sm:p-6" aria-labelledby="weekly-themes-title">
        <details className="group">
          <summary className="cursor-pointer list-none rounded-xl outline-none [&::-webkit-details-marker]:hidden focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-4">
            <div className="flex items-center justify-between gap-4">
              <h2 id="weekly-themes-title" className="font-display text-xl font-semibold tracking-tight">Weekly theme summary</h2>
              <span aria-hidden="true" className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-muted text-brand transition-colors group-hover:bg-brand/15"><svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="transition-transform duration-200 group-open:rotate-180 motion-reduce:transition-none"><path d="m5 7.5 5 5 5-5" /></svg></span>
            </div>
            <p className="mt-1 text-xs text-muted" aria-live="polite">{dateLabel(week, { day: 'numeric', month: 'short' })} – {dateLabel(shiftDay(week, 6), { day: 'numeric', month: 'short', year: 'numeric' })}</p>
            {data.available && <p className="mt-3 text-sm leading-6"><strong className="tabular-nums">{weekLogs.length}</strong> {weekLogs.length === 1 ? 'practice' : 'practices'}<span className="text-muted"> · {average === null ? 'No self-ratings yet' : `${average.toFixed(1)}/5 average self-rating`}</span></p>}
            <p className="mt-2 text-xs font-semibold text-brand"><span className="group-open:hidden">View themes</span><span className="hidden group-open:inline">Hide themes</span></p>
          </summary>
          <div className="mt-5 border-t border-border pt-5">
            <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted">{data.available ? `${activeDays} ${activeDays === 1 ? 'day' : 'days'} practised this week` : 'Choose a week'}</p><div className="flex gap-2"><button type="button" className={control} onClick={() => setWeek(shiftDay(week, -7))} disabled={!data.available || week <= range.from} aria-label="Previous practice week"><Chevron direction="left" /></button><button type="button" className={control} onClick={() => setWeek(shiftDay(week, 7))} disabled={!data.available || shiftDay(week, 7) >= range.until || shiftDay(week, 7) > data.today} aria-label="Next practice week"><Chevron direction="right" /></button></div></div>
        {data.available ? <>
