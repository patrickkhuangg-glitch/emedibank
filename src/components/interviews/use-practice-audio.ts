...
import {type UploadPhase} from '@/lib/interviews/upload-admission'
...
export function usePracticeAudio(station: InterviewStation, questionIndex = 0, daily = false) {
  ...
  async function save(durationSeconds: number, questionEvents: QuestionEvent[]) {
    ...
    busy.current = true; setSaving(true); setError(''); setProgress(0); setUploadPhase('waiting'); abort.current = new AbortController()
    try {
      attempt.current ??= crypto.randomUUID()
      if (!path.current) {
        const r = await fetch('/api/interviews/practice/recordings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: attempt.current, format: station.format, stationId: station.id, questionIndex, audioType: blob.current.type, daily }), signal: abort.current.signal })
        const data = await r.json()
        if (!r.ok) throw new Error(data.error || 'Saving could not start. Please retry.')
        path.current = data.audioPath
      }
      if (!uploaded.current) { await uploadInterviewMedia(blob.current, path.current!, setProgress, abort.current.signal,setUploadPhase); uploaded.current = true }
      setUploadPhase('saving')
      const r = await finaliseInterviewUpload(attempt.current, { durationSeconds, questionEvents }, abort.current.signal)
      const data = await r.json()
      if (!r.ok) throw new Error(data.error || 'Your audio uploaded, but saving could not finish. Retry safely.')
      setSavedId(attempt.current); setUploadPhase('saved')
  ...
  return { prepare, start, finish, reset, save, pause: () => abort.current?.abort(), url, error, saving, stopping, progress, uploadPhase, savedId, extension }
}
