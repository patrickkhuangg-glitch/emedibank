...
export function TranscriptHealthCheck(){
 ...
 const [result,setResult]=useState<{transcript:string;questions:string[];layout:TranscriptLayout;model:string;quality:{passed:boolean;matchedUnits:number;totalUnits:number}}|null>(null)
 ...
 return <details className="rounded-2xl border border-border p-5">
  ...
  {result&&<div className="mt-4"><p role="status">{result.quality.passed?'Grouping check passed':'Grouping needs attention'} · {result.quality.matchedUnits}/{result.quality.totalUnits} passages correctly grouped · {result.model}</p><InterviewTranscriptText text={result.transcript} questions={result.questions} layout={result.layout}/></div>}
 </details>
}
