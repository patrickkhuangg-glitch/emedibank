    ...
    <header className="sticky top-0 z-40 border-b border-border bg-surface/85 backdrop-blur">
      <Container className="flex h-16 items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/login" className="hidden rounded-full px-3 py-2 text-sm font-medium text-muted transition-colors hover:text-foreground sm:inline">
            Log in
          </Link>
          <Link
            href="/app"
...
function Dropdown({ label, children, leading, align = 'left' }: { label: string; children: ReactNode; leading?: ReactNode; align?: 'left' | 'right' }) {
  return (
    <div className="group relative">
      ...
      >
        <div className="overflow-hidden rounded-xl border border-border bg-surface p-1.5 shadow-lg">{children}</div>
      </div>
    </div>
  )
}
