...
export function SiteNav({ role, currentExamSlug, workspace = false }: { role: UserRole; currentExamSlug: string | null; workspace?: boolean }) {
  ...
  const active = (item: Item) => {
    if (item.label === 'Practice') return pathname.startsWith('/practice') || pathname.startsWith('/interviews/practice') || pathname.startsWith('/prototypes/interviews/practice')
    if (item.label === 'Live Practice') return pathname.startsWith('/interviews/live-practice')
    if (item.label === 'Mock exams') return pathname.startsWith('/mock')
    if (item.label === 'Dashboard') return pathname === item.href || pathname === '/app'
  ...
  return (
    <>
      <nav aria-label={`${roleLabel(role)} navigation`} className={`hidden items-center gap-0.5 whitespace-nowrap text-sm ${widerMenu ? 'xl:flex' : 'lg:flex'}`}>
        {items.map((item) => <PillLink key={item.label} {...item} active={active(item)} compact={widerMenu && !workspace} />)}
        <LogoutButton />
      </nav>
      <details className={`group relative ${widerMenu ? 'xl:hidden' : 'lg:hidden'}`}>
        ...
        <div className="eb-menu-surface absolute right-0 top-[calc(100%+0.75rem)] z-50 w-[min(20rem,calc(100vw-3rem))] overflow-hidden">
          ...
          <form action={signOutAction} className="mt-2 border-t border-border pt-2">
            <button type="submit" onClick={() => haptic(8)} className="eb-menu-item flex w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-left text-sm font-medium text-muted transition-colors hover:bg-surface-muted hover:text-foreground">
              <LogoutIcon />
              Log out
            </button>
          </form>
...
function navItems(role: UserRole, currentExamSlug: string | null): Item[] {
  ...
  if (currentExamSlug === 'interviews') return [
    ...
    { href: '/interviews/live-practice', label: 'Live Practice', icon: <LivePracticeIcon /> },
    ...
    { href: '/interviews/focus-shop', label: 'Focus Token Shop', icon: <TokenShopIcon /> },
    { href: '/account', label: 'Account', icon: <UserIcon /> },
  ]
  const practiceHref = currentExamSlug ? `/practice/${currentExamSlug}` : '/app'
  const mockHref = currentExamSlug ? `/mock/${currentExamSlug}` : '/app'
  return [
    { href: currentExamSlug ? '/dashboard' : '/app', label: 'Dashboard', icon: <GridIcon /> },
...
function PillLink({ href, label, icon, active, compact = false }: Item & { active: boolean; compact?: boolean }) {
  return <Link href={href} data-workspace-tab={href} prefetch={canPrefetchInterviewPage(href) ? true : undefined} onClick={() => haptic(8)} aria-current={active ? 'page' : undefined} className={`group inline-flex items-center gap-2 rounded-full px-2.5 py-2 font-medium transition-all duration-200 active:scale-95 ${active ? 'bg-surface-muted text-foreground shadow-sm' : 'text-muted hover:-translate-y-0.5 hover:bg-surface-muted/70 hover:text-foreground'}`}>{!compact && <PillIcon>{icon}</PillIcon>}<span>{label}</span></Link>
}

function MenuLink({ href, label, icon, active }: Item & { active: boolean }) {
  return <Link href={href} data-workspace-tab={href} prefetch={canPrefetchInterviewPage(href) ? true : undefined} onClick={() => haptic(8)} aria-current={active ? 'page' : undefined} className={`eb-menu-item flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-medium transition-colors ${active ? 'bg-brand-muted text-brand' : 'text-foreground hover:bg-surface-muted'}`}><PillIcon>{icon}</PillIcon><span>{label}</span>{active ? <span className="ml-auto h-1.5 w-1.5 rounded-full bg-brand" /> : null}</Link>
}
...
function LivePracticeIcon() { return <svg {...P}><circle cx="8" cy="9" r="3"/><circle cx="17" cy="9" r="3"/><path d="M2.5 20a5.5 5.5 0 0 1 11 0M12 20a5 5 0 0 1 10 0"/><path d="M11 4.5h3"/></svg> }
...
function TokenShopIcon() { return <svg {...P}><circle cx="12" cy="12" r="8" /><path d="M12 7.5v9M8.5 12h7" /><path d="M6.5 5.5 5 4m12.5 1.5L19 4" /></svg> }
