...
export function InterviewResourcesWorkspace() {
  ...
  return <WorkspaceShell title="Resources for your next session." description="Use a framework when it helps, then return to the prompt and make the answer your own.">
    <InterviewCytoCoach mood="studying" label="Cyto’s resource protocol" messages={[
      { title: 'Use the guide, then close the guide.', body: 'A framework should organise your thinking—not replace it. Read one idea, then test it out loud.' },
      { title: 'I highlighted everything. That was unhelpful.', body: 'Choose the single framework that matches today’s practice and leave the rest for later.' },
      { title: 'One resource. One response. One note.', body: 'That is a complete study loop. More tabs are not automatically more preparation.' },
    ]} />
    <section className="mt-7 grid gap-5 md:grid-cols-3">{resources.map(([title, body, icon]) => <article key={title} className="rounded-3xl border border-border bg-surface p-6 eb-soft"><span className="text-brand">{icon}</span><h2 className="mt-8 font-display text-2xl font-semibold tracking-tight">{title}</h2><p className="mt-3 text-sm leading-6 text-muted">{body}</p><a href="#resource-note" className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-brand">Open guide <ArrowIcon /></a></article>)}</section>
...
function WorkspaceShell({ title, description, children }: { title: string; description: string; children: ReactNode }) {
  return <main className="min-h-screen bg-background pb-16 text-foreground"><div className="page-frame page-shell"><header className="max-w-3xl"><h1 className="page-title">{title}</h1><p className="mt-4 text-base leading-7 text-muted sm:text-lg">{description}</p></header><div className="mt-9">{children}</div></div></main>
}

function FormatSwitch({ format, setFormat }: { format: Format; setFormat: (format: Format) => void }) { return <div className="flex w-fit rounded-full bg-surface-muted p-1" role="group" aria-label="Interview format">{(['mmi', 'panel'] as const).map((item) => <button type="button" key={item} onClick={() => setFormat(item)} className={`rounded-full px-4 py-2 text-sm font-semibold transition-[transform,background-color,color,box-shadow] duration-150 ease-[cubic-bezier(.23,1,.32,1)] active:scale-[.97] ${format === item ? 'bg-surface text-foreground eb-soft' : 'text-muted hover:text-foreground'}`}>{item === 'mmi' ? 'MMI practice' : 'Panel practice'}</button>)}</div> }
