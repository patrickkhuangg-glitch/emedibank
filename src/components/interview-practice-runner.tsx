...
import { validateMedia,type QuestionEvent } from '@/lib/interviews/media-validation'
...
export function InterviewPracticeRunner({station,enabled=false}:{station:InterviewStation;enabled?:boolean}) {
 ...
 function prepare(){if(timing.preparationSeconds===0){beginResponse();return}deadline.current=performance.now()+timing.preparationSeconds*1000;setSeconds(timing.preparationSeconds);setPhase('preparation')}
 ...
 const response=await finaliseInterviewUpload(current.attemptId,{durationSeconds:local.durationSeconds,questionEvents:events.current},abort.current.signal)
 ...
 {phase==='ready'&&<><p className="max-w-2xl leading-7 text-muted">Record with your camera and microphone, then preview before saving. Your recording is private. Self-review is free. You can use marking credits to request a comprehensive review of your strengths and areas to improve.</p><RecordingRetentionNotice /><p className="text-sm text-muted">Recording starts {timing.preparationSeconds > 0 ? 'after preparation' : 'when you begin the response'}.</p><button className={button} onClick={allowCamera}>Allow camera and microphone</button></>}
 {phase==='preview'&&<><video ref={preview} autoPlay muted playsInline className="max-h-[50vh] w-full rounded-2xl bg-black" aria-label="Live camera preview"/><p>{timing.preparationSeconds > 0 ? <>Check that you are visible and your microphone is connected. Preparation lasts {timing.preparationLabel}; your response is limited to {timing.responseLabel}.</> : <>Check that you are visible and your microphone is connected. Your response is limited to {timing.responseLabel} and starts when you continue.</>}</p><button className={button} onClick={prepare}>{timing.preparationSeconds > 0 ? 'Begin preparation' : 'Start response'}</button><button className="ml-3 rounded-full border px-5 py-3" onClick={discard}>Cancel</button></>}
