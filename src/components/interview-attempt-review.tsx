...
import {PanelReportView,type PanelReport} from '@/components/interviews/panel-feedback-view'
import { MMIFullMockFeedback } from '@/components/interviews/mmi-feedback-view'
import type { MMIReportEntry } from '@/lib/interviews/mmi-summary'
import { isMMIFeedback } from '@/lib/interviews/mmi-feedback'
...
import type { InterviewFeedbackData } from '@/lib/interviews/marking-validation'
...
export function InterviewAttemptReview({library,query,selected,selectedEntry,responseCount,credits,mockFeedback,panelReport,panelIndex=[]}:{panelReport?:PanelReport|null;panelIndex?:Array<{sessionId:string;status:string}>;library:ReturnType<typeof reviewLibrary>;query:Record<string,string|undefined>;selected:InterviewAttempt|null;selectedEntry?:ReviewEntry;responseCount:number;credits:number;mockFeedback?:MMIReportEntry[]}){
 return <main className="min-h-screen bg-background pb-16 text-foreground"><div className="page-frame page-shell">
  <header><h1 className="page-title">Mock Interviews</h1></header>
  <MockInterviewTabs active="review" />
  {<InterviewLibraryRefresh key={JSON.stringify(query)}/>}
  ...
  {!selected&&<RecordingRetentionNotice/>}
  {selected?<>
   ...
   {selectedEntry?.full&&!(panelReport&&['released','ungradable'].includes(panelReport.status))&&<div className="mt-4"><MockMarkingActions key={selectedEntry.id} sessionId={selectedEntry.id}/></div>}
   {panelReport&&<div className="mt-6"><PanelReportView report={panelReport}/></div>}
   <div className="mt-6 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
    <article className="min-w-0 rounded-2xl bg-surface p-5 sm:p-7">
     ...
     {selected.feedback&&!(mockFeedback&&isMMIFeedback(selected.feedback))&&<details open className="mt-5"><summary className="cursor-pointer py-2 font-semibold">Your feedback report</summary><InterviewFeedback feedback={selected.feedback} format={selected.format}/></details>}
     <RecordingRetentionNotice expiresAt={selected.recordingExpiresAt} pending={selected.markingPending} expired={selected.expired}/>
     {selected.expired?null:<InterviewMediaPlayer key={selected.id} attemptId={selected.id} url={selected.audioUrl} kind={selected.kind} events={selected.events}/>}
     <details className="border-t border-border py-3"><summary className="cursor-pointer py-2 text-sm font-semibold">Transcript</summary><InterviewTranscript key={selected.id} attemptId={selected.id} initialStatus={selected.transcriptionStatus} initialTranscript={selected.transcript} questions={selected.questions}/></details>
   ...
   <p className="mt-3 text-sm text-muted">MMI station <strong className="text-foreground">2 credits</strong> · Either full mock <strong className="text-foreground">12 credits</strong></p>
   {responseCount===0?<section className="mt-8 rounded-2xl bg-surface p-7"><h3 className="font-display text-xl font-semibold">Your recordings will appear here</h3><p className="mt-3 text-sm text-muted">Complete and save a mock interview to review it or request marking.</p><Link className="mt-5 inline-flex rounded-full bg-brand px-5 py-3 text-sm font-semibold text-brand-foreground" href="/interviews/mock-interviews">Start a mock interview</Link></section>:<>
    ...
    <section className="mt-3 overflow-hidden rounded-2xl bg-surface" aria-label="Saved interview attempts">
     {library.items.length===0?<p className="p-7 text-sm text-muted">No recordings match these filters. Try another search or clear the filters.</p>:<ul className="divide-y divide-border">{library.items.map(entry=>{
      ...
      const wholePanel=panelIndex.find(p=>p.sessionId===entry.id)
