...
const dir='artifacts/mock-review-polish-release',read=n=>JSON.parse(readFileSync(`${dir}/${n}.json`));
...
const result=await vercel(`/v13/deployments?teamId=${team}`,{method:'POST',body:JSON.stringify({name:previous.name,project:vercelProject,target:'production',files,meta:{purpose:'Polish mock review and reports with semantic colour, clearer borders and restrained parallax',sourceDeployment:previous.id}})});
