...
const dir='artifacts/mock-learning-release',read=n=>JSON.parse(readFileSync(`${dir}/${n}.json`));
...
const result=await vercel(`/v13/deployments?teamId=${team}`,{method:'POST',body:JSON.stringify({name:previous.name,project:vercelProject,target:'production',files,meta:{purpose:'Add time-pressure maps, student error classification and optional confidence calibration',sourceDeployment:previous.id}})});
