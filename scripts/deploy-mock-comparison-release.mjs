...
const dir='artifacts/mock-comparison-release',read=n=>JSON.parse(readFileSync(`${dir}/${n}.json`));
...
const result=await vercel(`/v13/deployments?teamId=${team}`,{method:'POST',body:JSON.stringify({name:previous.name,project:vercelProject,target:'production',files,meta:{purpose:'Replace mock review time summaries with previous-average score comparisons',sourceDeployment:previous.id}})});
