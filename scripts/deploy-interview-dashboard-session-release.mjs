...
assert(existsSync(`${artifactDir}/migration.json`), 'The single-device session migration has not been applied and verified')
...
const result = await vercel(`/v13/deployments?teamId=${team}`, {
  ...
  body: JSON.stringify({
    ...
    target: 'production',
    files,
    meta: { purpose: 'Interview dashboard clarity, daily streak deadline and single-device sessions', sourceDeployment: baseline.id },
  }),
})
