...
const dir='artifacts/qr-480-upload',source='../output/ucat-qr-120-sets',subtest='86a52ac4-f2b9-40b8-997b-5e44de337f92'
const content=JSON.parse(readFileSync(`${source}/question-bank.json`));assert.equal(content.question_count,480)
assert.equal(content.sets.length,120)
assert.equal(new Set(content.sets.flatMap(s=>s.questions.map(q=>q.id))).size,480)
for(const set of content.sets){assert.equal(set.questions.length,4);assert.ok(['Tables','Diagrams','Complex','Text only'].includes(set.tag));for(const item of set.questions){assert.equal(Object.keys(item.options).join(''),'ABCDE');assert.equal(new Set(Object.values(item.options)).size,5);assert.equal(item.options[item.correct],item.answer);assert.ok(item.explanation.includes(`Correct answer: ${item.correct} - ${item.answer}`));assert.equal(Object.keys(item.distractors).length,4)}}
...
let payload
if(existing.length){assert.equal(existing.length,480,'Partial existing collection must be audited');assert.ok(existsSync(`${dir}/manifest.json`),'Existing collection needs verification against its upload manifest');payload=JSON.parse(readFileSync(`${dir}/manifest.json`))}
else{
 ...
 for(const [i,set] of content.sets.entries()){
  ...
  payload.stimuli.push({id:stimulus_id,subtest_id:subtest,title:`Studocyte QR Set ${set.number}: ${set.title}`,data,sort_order:order+i})
  for(const item of set.questions){
   const id=randomUUID()
   payload.questions.push({id,subtest_id:subtest,stimulus_id,kind:'single_best_answer',topic:null,stem:item.stem,data:{source_id:item.id,source_set:set.key,source_title:set.title,source_collection:'STUDOCYTE-QR-LONG01',source_volume:set.volume,reasoning_family:set.family},explanation_text:item.explanation,difficulty:item.difficulty,sort_order:order+item.number-1,published:true,tags:[set.tag]})
 ...
 assert.equal(payload.stimuli.length,120);assert.equal(payload.questions.length,480);assert.equal(payload.options.length,2400)
 writeFileSync(`${dir}/manifest.json`,JSON.stringify(payload,null,2)+'\n')
 const query=`DO $import$ BEGIN
 PERFORM pg_advisory_xact_lock(hashtext('studocyte-qr-approved-long-01'));
 ...
 END $import$;`
 console.log(JSON.stringify({stage:'publishing',questions:480,sets:120}));await sql(query,false)
}
const after=await fetchBank(),sample=after.filter(r=>identifiers.includes(r.data?.source_id));assert.equal(sample.length,480)
...
const result={verifiedAt:new Date().toISOString(),publishedQuestions:480,sets:120,options:2400,diagrams:20,tables:52,allExplanationsMatch:true,existingQuestionsPreserved:true,totalQRQuestions:after.length,totalPublishedQRQuestions:after.filter(r=>r.published).length,alreadyPresent:existing.length>0}
