...
const directory = 'artifacts/question-loading-timer'
...
const additions = ['src/lib/practice/load-question-section.ts', 'src/components/question-loading.tsx']
...
const result = await vercel(`/v13/deployments?teamId=${team}`, {
  ...
  body: JSON.stringify({
    ...
    target: 'production',
    files,
    meta: { actor: 'codex', purpose: 'Wait for question data and diagrams before starting every section timer; animated loading and retry', sourceDeployment: previous.id },
  }),
})
