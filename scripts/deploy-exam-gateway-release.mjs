...
const artifactDir = 'artifacts/exam-gateway-release'
...
const result = await vercel(`/v13/deployments?teamId=${team}`, {
  ...
  body: JSON.stringify({
    ...
    target: 'production',
    files,
    meta: { purpose: 'Full-screen exam selection before mounting the selected workspace', sourceDeployment: baseline.id },
  }),
})
