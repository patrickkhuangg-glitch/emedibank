...
const checks = await Promise.all(['/', '/login', '/app', '/dashboard', '/practice/ucat', '/practice/gamsat', '/practice/isat', '/interviews', '/api/health', '/interview-trial', '/pricing', '/prototypes/workspace'].map(async path => {
  ...
  else if(['/app', '/dashboard', '/practice/ucat', '/practice/gamsat', '/practice/isat', '/interviews'].includes(path)) { assert([303,307,308].includes(response.status)); assert(result.location?.includes('/login')); }
