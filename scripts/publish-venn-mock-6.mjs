...
const source='../output/ucat-venn-mock-6',dir='artifacts/venn-mock-6-release'
const collection='ucat-dm-venn-mock-6-2026-09-09'
const reserveKey='reserved-full-mock-dm-venn-1'
const reserve=[1,2,3,4,5,6]
...
assert.equal(audit.source_sha256,hash);assert.equal(items.length,6)
...
assert.ok(existing.length===0||existing.length===6,'Partial import requires inspection')
let manifest
...
assert.equal(manifest.questions.filter(x=>x.published&&!x.data.mock_only).length,0)
assert.equal(manifest.questions.filter(x=>!x.published&&x.data.mock_only).length,6)
assert.equal(manifest.assignments.length,6);assert.equal(manifest.options.length,24)
console.log(JSON.stringify({stage:'prepared',practiceQuestions:0,reservedMockQuestions:6,reservedSourceQuestions:reserve,alreadyPresent:existing.length}))
...
assert.deepEqual(counts,{practice:0,reserved:6})
const result={verifiedAt:new Date().toISOString(),sourceHash:hash,uploaded:6,publishedPractice:0,reservedFullMock:6,reservedSourceQuestions:reserve,reserveKey,options:24,allStoredContentMatches:true,existingContentPreserved:true,existingAssignmentsPreserved:true}
