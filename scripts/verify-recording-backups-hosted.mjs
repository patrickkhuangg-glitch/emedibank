...
import {stagingClients,stagingSql,quoteSql} from './lib/staging-operator.mjs';
import {loadModule} from '../tests/helpers/load-module.mjs';
import {restoreRecordingBackup} from './lib/recording-backup-restore.mjs';
const staging=process.argv.includes('--authorised-staging-backup-test');
const operatorClients=staging?stagingClients:clients,operatorSql=staging?stagingSql:sql,operatorQuote=staging?quoteSql:q;
const bucket=staging?'studocyte-interview-backups-staging':'studocyte-interview-backups';
...
mkdirSync('artifacts/ongoing-backups',{recursive:true});
function save(){writeFileSync(`artifacts/ongoing-backups/${staging?'staging-':''}hosted-verification.json`,JSON.stringify({run,staging,synthetic:true,checks,cleaned,failure,fixtures:fixtures.map(({id,kind})=>({id,kind}))},null,2)+'\n')}
function checked(value,message){if(!value)throw Error(message);checks.push(message);save();console.log('PASS '+message)}
function data(result,label='operation'){if(result.error)throw Error(`${label} failed (${result.error.code??result.error.status??'unknown'})`);return result.data}
async function verify(credentials){
 const {admin:db}=await operatorClients();
 ...
 try{
  ...
  data(await db.rpc('authorize_signup',{p_email:email,p_token_hash:createHash('sha256').update(token).digest('hex')}),'authorise synthetic signup');
  userId=data(await db.auth.admin.createUser({email,password:randomBytes(32).toString('base64url')+'Aa9!',email_confirm:true,user_metadata:{signup_authorization:token,hosted_test_run:run,full_name:'Synthetic backup verification'}}),'create synthetic user').user.id;save();
  for(const kind of ['audio','video']){
   ...
   await operatorSql(`insert into public.interview_attempts(id,user_id,format,station_id,station_title,recording_path,recording_mime_type,media_kind,upload_status,duration_seconds,transcript,transcription_status) values(${operatorQuote(id)},${operatorQuote(userId)},'mmi','synthetic-backup','Synthetic backup transport check',${operatorQuote(path)},${operatorQuote(kind+'/webm')},${operatorQuote(kind)},'ready',60,'Synthetic transcript must remain.','ready')`,false);
   data(await db.storage.from('interview-recordings').upload(path,bytes,{contentType:kind+'/webm'}),`upload ${kind} fixture`);
  }
  data(await db.from('interview_backup_health').update({enabled:true,last_success_at:new Date().toISOString(),last_error:null}).eq('singleton',true),'enable backup health');
  let ready=false;
  ...
  await operatorSql(`update public.interview_attempts set marking_status='awaiting_review' where id=${operatorQuote(fixtures[1].id)}`,false);
  await operatorSql(`begin;set local session_replication_role='replica';update interview_attempts set recording_expires_at=now()-interval '1 hour' where user_id=${operatorQuote(userId)};set local session_replication_role='origin';commit;`,false);
  ...
  await operatorSql(`update public.interview_attempts set marking_status='released' where id=${operatorQuote(fixtures[1].id)}`,false);
  await worker.runRecordingBackups();
  ...
  const attempts=await operatorSql(`select transcript from public.interview_attempts where id in (${fixtures.map(f=>operatorQuote(f.id)).join(',')})`);
