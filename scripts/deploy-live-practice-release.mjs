...
assert(existsSync(`${artifactDir}/migration.json`), 'The Live Practice database migration has not been applied and verified')
...
const result = await vercel(`/v13/deployments?teamId=${team}`, {
  ...
  body: JSON.stringify({
    ...
    target: 'production',
    files,
    meta: { purpose: 'Private two-person interview Live Practice rooms', sourceDeployment: baseline.id },
  }),
})
