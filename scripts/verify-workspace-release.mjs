...
const checks = await Promise.all(['/', '/login', '/dashboard', '/practice/ucat', '/interviews', '/api/health', '/prototypes/workspace'].map(async path => {
  ...
  else if(['/dashboard', '/practice/ucat', '/interviews'].includes(path)) { assert([303,307,308].includes(response.status)); assert(result.location?.includes('/login')); }
