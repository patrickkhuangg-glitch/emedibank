...
const outputDirectory = 'artifacts/recognising-assumptions-category'
...
assert.equal((productionSource.match(/'Interpreting Information'/g) ?? []).length, 1, 'Expected one Interpreting Information category in production')
assert.equal((productionSource.match(/'Recognising Assumptions'/g) ?? []).length, 0, 'Recognising Assumptions already exists in production')

const updatedSource = productionSource.replace("      'Interpreting Information',\n", "      'Recognising Assumptions',\n")
assert.ok(!updatedSource.includes("'Interpreting Information'"))
assert.equal((updatedSource.match(/'Recognising Assumptions'/g) ?? []).length, 1)
...
const release = await vercel(`/v13/deployments?teamId=${team}`, {
  ...
  body: JSON.stringify({
    ...
    meta: {
      actor: 'codex',
      purpose: 'Correct the Decision Making category from Interpreting Information to Recognising Assumptions',
