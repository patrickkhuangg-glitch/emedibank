...
for (const [file, content] of Object.entries(changes)) {
  assert.equal(readFileSync(`${sourceDir}/${file}`, 'utf8'), content, `Built source changed: ${file}`)
}
for (const { file, sha } of manifest) {
  if (file in changes) continue
  const digest = createHash('sha1').update(readFileSync(`${sourceDir}/${file}`)).digest('hex')
  assert.equal(digest, sha, `Staging baseline changed: ${file}`)
}
...
const currentDeploymentId = staging?.deploymentId ?? staging?.deployment?.id
assert.equal(currentDeploymentId, baseline.id, 'Staging changed; prepare again before publishing')

const files = manifest.map(({ file, sha, mode }) => file in changes
  ? { file, data: changes[file], encoding: 'utf-8' }
  : { file, sha, mode })
for (const [file, data] of Object.entries(changes)) {
  if (!manifest.some((entry) => entry.file === file)) files.push({ file, data, encoding: 'utf-8' })
}
...
const result = await vercel(`/v13/deployments?teamId=${team}`, {
  ...
  body: JSON.stringify({
    ...
    meta: {
      actor: 'codex',
      purpose: 'one-off-interview-plans-and-review-credit-add-ons',
