...
const sourceDir = resolve('.vercel/interview-rewards-release')
const paths = [
  'src/app/(app)/interviews/page.tsx',
  'src/app/(app)/interviews/practice/recordings/page.tsx',
  'src/app/(app)/interviews/practice/session/page.tsx',
  'src/app/api/interviews/practice/[activityId]/route.ts',
  'src/app/api/interviews/practice/recordings/route.ts',
  'src/app/api/interviews/practice/route.ts',
  'src/app/api/interviews/progression/quest/route.ts',
  'src/app/api/interviews/progression/route.ts',
  'src/app/prototypes/interviews/page.tsx',
  'src/components/interviews/practice-progress.tsx',
  'src/components/interviews/progression-dashboard.module.css',
  'src/components/interviews/progression-dashboard.tsx',
  'src/components/interviews/progression-review.tsx',
  'src/components/interviews/rehearsal-runner.tsx',
  'src/components/interviews/use-practice-audio.ts',
  ...
  'src/lib/interviews/video-validation.ts',
...
for (let index = 0; index < manifest.length; index += 12) {
  await Promise.all(manifest.slice(index, index + 12).map(async (entry) => {
    ...
    if (createHash('sha1').update(content).digest('hex') !== entry.sha) {
      throw new Error(`Production source hash mismatch: ${entry.file}`)
    }
