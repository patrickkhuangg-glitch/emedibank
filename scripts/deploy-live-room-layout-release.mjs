...
const artifactDir = 'artifacts/live-room-layout-release'
...
const result = await vercel(`/v13/deployments?teamId=${team}`, {
  ...
  body: JSON.stringify({
    ...
    target: 'production',
    files,
    meta: { purpose: 'Place the live station prompt first and move video to a compact right rail', sourceDeployment: baseline.id },
  }),
})
