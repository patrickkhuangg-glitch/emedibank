...
import * as emailLinks from '../src/lib/auth/email-links'
import { loadModule } from './helpers/load-module.mjs'

function fixture(error: { code: string; status: number } | null = null, throws = false) {
  let calls = 0
  const verify = async () => {
    calls++
    if (throws) throw new Error('Connection unavailable')
  ...
  const route = loadModule('src/app/auth/confirm/route.ts', {
    ...
    '@/lib/auth/single-device': { claimSingleDeviceSession: async () => true },
    '@/lib/supabase/server': { createClient: async () => ({ auth: { verifyOtp: verify, exchangeCodeForSession: verify }, from: () => chain }) },
  }, { TextDecoder }) as { GET: (request: Request) => Response; POST: (request: Request) => Promise<Response> }
