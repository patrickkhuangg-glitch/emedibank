...
test('introduction starts at dashboard, follows the workflow, and never opens timed or recording sessions',()=>{
 assert.equal(INTRO_STEPS[0].path,'/interviews');assert.equal(INTRO_STEPS.length,14)
 assert.equal(new Set(INTRO_STEPS.map(step=>step.target)).size,INTRO_STEPS.length)
 assert.match(INTRO_STEPS[0].body,/previous practice/)
 assert.ok(INTRO_STEPS.every(s=>introductionAllowed(s.path)))
 assert.ok(INTRO_STEPS.every(s=>s.tabPath.startsWith('/interviews')))
 assert.deepEqual(INTRO_STEPS.filter(s=>s.path==='/interviews/live-practice').map(s=>s.target),['live-connect','live-station','live-room'])
 ...
 for(const value of [undefined,null,false,{},'unknown','started'])assert.equal(hasSeenIntroduction(value),false)
 for(const value of ['skipped','completed'])assert.equal(hasSeenIntroduction(value),true)
 for(const value of [null,'invalid','{"version":4,"step":99}','{"version":1,"step":1}','{"version":4,"step":-1}'])assert.equal(readIntroductionStep(value),null)
 assert.equal(readIntroductionStep('{"version":4,"step":3}'),3)
})
test('story validation trims and limits input, rejects invalid themes and ignores owner/version payloads',()=>{
 ...
 assert.deepEqual(value,draft);assert.equal(validateStory({...draft,reflection:''}).reflection,'');assert.throws(()=>validateStory({...draft,context:'',actions:'',reflection:''}));assert.throws(()=>validateStory({...draft,theme:'private-script'}));assert.throws(()=>validateStory({...draft,context:'x'.repeat(2001)}))
})
