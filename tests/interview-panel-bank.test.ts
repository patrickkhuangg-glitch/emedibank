...
import { getInterviewQuestions, getInterviewTiming, getInterviewTimingLabel, getPracticeQuestionIndex } from '../src/lib/interviews/timing'
...
test('individual panel practice starts directly with a two-minute response', () => {
 const timing = getInterviewTiming('panel')
 assert.equal(timing.preparationSeconds, 0)
 assert.equal(timing.responseSeconds, 120)
 assert.equal(getInterviewTimingLabel('panel'), '2 min response')
 assert.equal(getInterviewTimingLabel('mmi'), '2 min prep · 8 min response')
})

test('panel bank preserves the 160 supplied questions and the additional high-stress question', () => {
 ...
 assert.ok(panel.every(s => s.questions.length === (s.id === 'panel-resilience' ? 6 : 5) && new Set(s.questions).size === s.questions.length))
 assert.equal(panel.find(s=>s.id==='panel-resilience')!.questions[5], 'Tell me about a time where you have coped with a high stress situation and how you got through it.')
 const bank = panel.map(s => ({number:s.panelThemeNumber,title:s.title,questions:s.questions.slice(0,5),preparation:s.preparation}))
 ...
 assert.equal(mockOptions().filter(option => option.format === 'panel').length, 0)
 assert.deepEqual(panel.slice(0, 4).map(s => s.id), ['panel-motivation','panel-service','panel-resilience','panel-teamwork'])
})

test('full panels always open with motivation and balance five paired themes over exactly 20 minutes', () => {
 ...
 for (let seed = 1; seed <= 1000; seed++) {
  ...
  assert.equal(steps.reduce((total, step) => total + step.preparationSeconds + step.responseSeconds, 0), 1200)
  ...
  for (let i = 0; i < steps.length; i++) {
   ...
   assert.equal(step.preparationSeconds, 0);assert.equal(step.responseSeconds, 120)
   seen.add(`${step.stationId}:${step.questionIndex}`)
   const view = mockView(ticket, 1000 + i * 120000)
   assert.equal(view.index, i)
   assert.deepEqual(view.questions, [station.questions[step.questionIndex]])
   assert.ok(!('steps' in view))
  }
  assert.equal(mockView(ticket, 1201000).phase, 'complete')
 }
 assert.equal(seen.size, 161, 'Every question remains reachable across balanced mocks')
...
test('individual practice accepts every panel question and preserves old links while rejecting invalid selections', () => {
 for (const station of panel) {
  ...
  for (let index = 0; index < station.questions.length; index++) {
  ...
  for (const invalid of [-1, station.questions.length, 1.5, '', '1x', null, {}, ['2'], Infinity]) assert.equal(getPracticeQuestionIndex(station, invalid), null)
 }
