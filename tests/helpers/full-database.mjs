...
export async function fullDatabase() {
  // Fixture accounts default to an Auth-trusted Google identity. Dedicated signup
  // tests supply explicit app metadata and exercise the email ticket gate.
  const db=new PGlite()
  await db.exec(`create role anon; create role authenticated; create role service_role bypassrls;
    create schema auth; create schema storage;
    create table auth.users(id uuid primary key,email text,raw_app_meta_data jsonb default '{"provider":"google"}',raw_user_meta_data jsonb default '{}');
