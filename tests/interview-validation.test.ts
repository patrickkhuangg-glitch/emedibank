...
test('media allowlists, codecs, size and duration bounds',()=>{
 ...
 assert.equal(validateDuration(300,'panel',300),300)
 assert.throws(()=>validateDuration(311,'panel',300))
...
test('canonical station snapshot and bounded question offsets',()=>{
 ...
 assert.equal((stationSnapshot('panel','panel-motivation',0,'daily').timing as {response_seconds:number}).response_seconds,300)
...
test('retention preserves pending reviews and expired media is not reprocessed',()=>{
 ...
 assert.equal(retentionEligible({...base,marking_status:'released',released_at:'2026-09-01'},now),true)
 assert.equal(retentionEligible({...base,media_kind:'audio'},now),true)
 assert.equal(retentionEligible({...base,video_deleted_at:'2026-09-01'},now),false)
 assert.equal(retentionDays(undefined),7);assert.equal(retentionDays('invalid'),7);assert.equal(retentionDays('30'),7);assert.equal(retentionDays('0'),7)
})
