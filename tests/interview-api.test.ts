...
mock('../src/lib/interviews/trial.ts',{interviewAccess:async()=>({kind:'full'}),requireInterviewPractice:async()=>({kind:'full'})})
mock('../src/lib/interviews/trial-provider.ts',{admitTrialProvider:async()=>{}})
...
function request(body:unknown={}){return new Request('http://localhost/api',{method:'POST',body:JSON.stringify(body),headers:{'Content-Type':'application/json',Origin:'http://localhost'}})}
...
test('durable worker: missing configuration, transcript persistence before audio deletion, malformed AI and timeout',async()=>{
 ...
 rpcHandler=(name)=>name==='claim_fair_interview_job'?{queue:'attempt',job}:true
 ...
 rpcHandler=(name)=>name==='claim_fair_interview_job'?{queue:'attempt',job}:name==='consume_interview_transcription'?false:true
 ...
 await processInterviewJob();assert.equal(calls.at(-1)!.args.p_code,'invalid_mmi_specification')
...
test('timed mock API validates selections, gates prompts and creates canonical retryable responses',async()=>{
 ...
 try{
  ...
  user={id:'student-a'}
  assert.equal((await mock.POST(request({action:'start',selection:{format:'panel',mode:'individual',selectionId:'panel-motivation:2'}}))).status,400)
  const {token,ticket:panelTicket}=startMockSession(user.id,{format:'panel',mode:'full'},Date.now()-121000)
  assert.equal(panelTicket.steps.reduce((n,s)=>n+s.preparationSeconds+s.responseSeconds,0),1200)
  ...
  const result=await created.json(),snapshot=inserted!.station_snapshot as {question_index:number;timing:{response_seconds:number};mock_session:{mode:string;total:number}}
  assert.equal(snapshot.question_index,panelTicket.steps[0].questionIndex)
  assert.equal(snapshot.timing.response_seconds,120)
  assert.equal(snapshot.mock_session.total,10);assert.equal(snapshot.mock_session.mode,'full')
  assert.deepEqual(inserted!.questions,[INTERVIEW_STATIONS.find(s=>s.id==='panel-motivation')!.questions[panelTicket.steps[0].questionIndex]])
...
test('the existing worker transcribes practice audio and retains the playback original without charging credits',async()=>{
 ...
 rpcHandler=name=>name==='claim_fair_interview_job'?{queue:'attempt',job}:true;media=true
