...
const {uploadInterviewMedia,uploadErrorMessage}=loadModule('src/lib/interviews/video-upload.ts',{
 ...
 './upload-admission':{withUploadSlot:async(_id:string,signal:AbortSignal,_phase:unknown,upload:(s:AbortSignal)=>Promise<void>)=>upload(signal),uploadRetryDelays:()=>[0,1]},
},{AbortController,AbortSignal,setTimeout,clearTimeout,process:{env:{NEXT_PUBLIC_SUPABASE_URL:'https://project.supabase.co'}}}) as typeof import('../src/lib/interviews/video-upload')
