...
const shared={'@/lib/interviews/trial':{requireInterviewPractice:async()=>({kind:'full'})},'@/lib/auth/dal':auth,'@/lib/interviews/api':api,'@/lib/supabase/server':{createClient:async()=>db},'@/lib/interviews/stories':{validateStory,validStoryId}}
...
test('introduction preference authenticates, limits fields, and reports unavailable account persistence',async()=>{
 ...
 assert.equal((await intro.POST(req({status:'skipped',role:'admin'}))).status,200);assert.equal(JSON.stringify(saved),JSON.stringify({data:{interview_intro_v4:'skipped'}}))
