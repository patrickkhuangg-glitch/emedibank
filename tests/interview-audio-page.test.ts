...
const rows = Array.from({length:12},(_,i)=>({ id:`audio-${i}`,user_id:'owner',media_kind:'audio',format:'panel',station_title:`Practice ${i}`,created_at:'2026-09-06T14:30:00Z',duration_seconds:30,upload_status:'ready',marking_status:null,recording_expires_at:'2099-01-01',video_deleted_at:null,recording_path:`owner/audio-${i}/practice.webm`,question_events:[{question_index:0,offset_seconds:0}],transcription_status:'ready',transcript:'PRIVATE_AUDIO_TRANSCRIPT' }))
let signed = 0
...
mock('../src/lib/auth/dal.ts',{requireUser:async()=>({id:'owner'}),getProfile:async()=>({mmi_credits:2})})
...
test('practice library paginates metadata privately; only a selected audio response loads playback and transcript',async()=>{
 ...
 assert.doesNotMatch(selected,/Submit for marking/)
